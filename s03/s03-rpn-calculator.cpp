...
auto evaluate_devision(std::stack<double>& stack) -> void
{
    if (stack.size() < 2 || stack.top() == 0) {
...
auto evaluate_int_devision(std::stack<double>& stack) -> void
{
    if (stack.size() < 2 || stack.top() == 0) {
        throw std::logic_error{"not enough or wrong operands for //"};
    }
...
{
    if (stack.size() < 1) {
        throw std::logic_error{"not enough or wrong operands for sqrt"};
    }
    auto const a = pop_top(stack);
    stack.push(sqrt(a));
}

auto evaluate_circle(std::stack<double>& stack) -> void
{
    if (stack.size() < 1) {
        throw std::logic_error{"not enough or wrong operands for circle"};
    }
    auto const a = pop_top(stack);
    stack.push(M_PI * a * 2);
}
    ...
    for (auto const each : args) {
        try {
            if (each == "p") {
                ...
                evaluate_devision(stack);
            } else if (each == "//") {
                evaluate_int_devision(stack);
            } else if (each == "%") {
